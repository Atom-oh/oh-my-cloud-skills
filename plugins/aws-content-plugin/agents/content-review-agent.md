...
- **문서/다이어그램의 예시 사설 IP·CIDR** — 아키텍처 문서의 VPC/서브넷 표기(예: `10.0.0.0/16`)처럼 설계를 설명하는 사설 대역 표기는 finding 아님 (실제 내부 시스템의 구체 IP 노출과 구별).
- **동종 반복 합산** — High·Medium 등급에서 같은 유형(같은 이메일, 같은 서브넷 대역 등)이 문서 전반에 반복 노출되면 서로 다른 노출 지점 각각이 아니라 **1건의 finding으로 합산**해 기록합니다 (감지 자체는 여러 곳이어도 근본 문제는 하나이므로 — 그래야 반복 노출 하나가 Warning band를 독점적으로 밀어 올리지 않습니다). 서로 다른 이메일·IP가 각각 등장하면 그건 별개 finding입니다.
...
각 카테고리는 만점에서 시작합니다. 카테고리 안에 Critical 결함이 있으면 그 카테고리는 0점, 결함이 전혀 없으면 만점입니다. 그 사이는 **결함 1건당 카테고리 만점의 약 1/4을 감점**하는 고정 비율로 계산하고(같은 카테고리에 결함이 여러 건이면 누적, 0점 하한), 재량은 “이게 정말 이 카테고리의 목적을 훼손하는 결함인가”를 판단하는 데만 쓰지 감점 크기를 임의로 정하는 데는 쓰지 않습니다 — 모든 감점은 리포트의 구체적 finding(위치+인용)에 연결되어야 하고, finding 없는 감점은 없습니다. 이 고정 비율 덕분에 동일한 finding 집합이면 항상 동일한 카테고리 점수가 나옵니다(tie-breaking이 아니라 산식 자체의 결과). Verdict의 PASS/REVIEW/FAIL 경계는 아래 Verdict 표(85/70, 90점 스케일 77/63)가 유일한 기준입니다 — 여기서 별도 경계를 재정의하지 않습니다.
