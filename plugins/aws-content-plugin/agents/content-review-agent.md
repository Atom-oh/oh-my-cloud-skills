...
- **문서/다이어그램의 예시 사설 IP·CIDR** — 아키텍처 문서의 VPC/서브넷 표기(예: `10.0.0.0/16`)처럼 설계를 설명하는 사설 대역 표기는 finding 아님 (실제 내부 시스템의 구체 IP 노출과 구별). 같은 유형의 Medium finding이 문서 전반에 반복되면 1건으로 합산해 기록.
