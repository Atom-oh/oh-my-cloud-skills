...
the LLM gets subtly wrong, which is what makes output look amateur:
...
It prints a layout SCORE (0-100) and exits non-zero below the threshold so it can be
used as a hard pre-export gate. Tunables match references/design-tokens.md.
...
def analyze(text):
    ...
    score = max(0, 100 - sum(w for w, _ in findings))
    return {
        "score": score,
        "counts": {"vertices": len(vertices), "icons": len(icons), "edges": len(edges)},
        "findings": [m for _, m in findings],
    }
...
def main():
    ...
    mark = "✅" if ok else "❌"
    print(f"{mark} {path}: layout score {result['score']}/100 (gate {threshold}) "
          f"— icons={c['icons']} edges={c['edges']} vertices={c['vertices']}")
    for m in result["findings"]:
        print(f"   • {m}")
    if not ok:
        print("   → Below the layout gate. Fix the above (align to grid, fix containment/"
              "overlap, even out spacing, cut edges) before exporting. See design-tokens.md.")
