...
> 💡 **먼저 스펙 생성기를 고려하세요.** VPC/Multi-AZ·서버리스·멀티리전·하이브리드 패턴이라면
> 좌표를 손으로 찍지 말고 **`scripts/layout_aws.py`** 로 YAML 스펙에서 생성하세요 — 아래 모범사례
> (78px 균일·미러 대칭·여백·엣지 직교)가 전부 자동 적용되고 게이트를 통과합니다.
> 예시: `examples/`. 이 문서는 생성기에 안 맞는 비정형 구조를 손으로 그릴 때의 기준입니다.
