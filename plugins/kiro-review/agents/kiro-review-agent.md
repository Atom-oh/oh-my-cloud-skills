...
2. **Kiro Code Review Delegation** — `kiro-cli chat --no-interactive`(headless)로 일반 코드 리뷰 위임
...
5. **Spec-Driven Validation** — `kiro-cli` headless 호출(또는 대화형 `/kiro-cli:spec`)로 EARS 요구사항 기반 설계 추적성 검증
