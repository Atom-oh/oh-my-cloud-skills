...
# superseded it; ADR-010) — but a user-run `gemini` still auto-loads a repo-root GEMINI.md
# an older co-agent version may have left. We don't manage it, but if one is present we
# still secret-scan it and suggest deletion.
    ...
    for name in RESIDUAL_SCAN:
        ...
        if SECRET_RE.search(body):
            problems.append(f"{name}: residual unmanaged file with a possible secret — "
                            f"delete it (co-agent no longer uses gemini, but a user-run "
                            f"gemini CLI would auto-load it into a third-party AI)")
        else:
