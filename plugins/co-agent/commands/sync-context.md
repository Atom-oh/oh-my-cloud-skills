...
2. **Distill** a lean core covering only what helps an external reviewer judge a diff:
   language/stack/runtime · build/test/lint commands (copy-paste ready) · naming
   conventions + banned patterns (e.g. the global AWS security mandates) · architectural
   boundaries (what may import what; where logic belongs) · PR/review expectations
   (test-coverage bar, error-handling style, security rules) · a short review checklist +
   known false-positives to suppress. Omit transient project state, release mechanics,
   tool internals, and exhaustive file inventories. **Never include secrets** — these
   files go to third-party AIs.
