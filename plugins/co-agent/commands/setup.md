...
Produce the panel-readiness summary at `.claude/co-agent-panel.local.json`: which peers
are reachable, through which access path, and whether each is actually usable headlessly.
The consumers are the autonomous flows (consensus/harness) that consult it before fanning
out. Excellent means every peer's real state is probed rather than assumed, and the user
leaves having made a conscious choice about the pre-push gate.
...
2. For each peer whose row shows `raw` + a `suggest_install` repo (e.g. codex →
   `openai/codex-plugin-cc`), offer the official plugin once via `AskUserQuestion`, install
...
3. If codex specifically is `none` and `npm` is available, offer the CLI install once
...
6. If at least one peer is gate-eligible, offer the pre-push lens gate ONCE via
   ...
   **≥2 lenses flagging → BLOCKED** (push stops); **exactly 1 → CHAIR JUDGMENT REQUIRED**
   (push still stops with exit 2, but the finding is uncorroborated — the host reads it
   against the actual change and may bypass); **0 → PASS**. Also state the honest limit:
   a push the gate genuinely cannot review (no peer reachable, no parseable verdict, or
   the command doesn't match a plain same-repo `git push`) fails **open** — the push
   proceeds — by design, so an offline peer can never permanently block pushing. First
   check whether kiro's `review.on_push` is already on for this repo
   (`python3 "${CLAUDE_PLUGIN_ROOT}/../kiro/skills/kiro-delegate/scripts/kiro_config.py"
   review-on-push --root .` — best-effort, kiro may not be installed; ignore a failure
   silently) — if it is, put that fact in the option description ("kiro's own pre-push
   gate is already on for this repo; turning this on too means every push runs both — not
   recommended") rather than refusing the option.
