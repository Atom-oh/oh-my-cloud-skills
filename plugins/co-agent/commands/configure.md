...
2. **`set <ai> <key> <value>`** → validate + write to `.claude/co-agent.local.json`,
   ...
   python3 "$H" set kiro-cli  model claude-opus-5 # Kiro model (see `kiro-cli chat --list-models`)
   ...
   python3 "$H" set kiro-cli models claude-opus-5,minimax-m2.5  # multi-model list
