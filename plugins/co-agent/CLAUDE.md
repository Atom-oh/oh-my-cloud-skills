...
**모델 티어링(역할별 배치)**: chair = 호스트 모델(`/model opusplan` 또는 chair 서브에이전트 `model: opus`) · find 패널 = `profile deep` 저비용 breadth · verify 패널 = `--profile default` 단일 최강 모델(하이브리드 게이트가 자동 적용) · implementer = `set harness implementer_model <m>` / `implementer_effort <e>`(codex 전용; 미설정 시 패널 폴백, **impl-flags 쓰기 경로에만** 적용). 상세: `commands/configure.md` "모델 티어링", `references/hybrid-gate.md` "Role tiering".
