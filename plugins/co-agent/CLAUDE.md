...
- **명령 매칭**: `gh pr create`가 명령 경계(줄 시작 또는 `;`/`&`/`|`/`&&` 뒤)에 올 때만 — `env `·`VAR=val` prefix와 `gh -R o/r pr create`처럼 gh~pr 사이 플래그도 허용. `echo "gh pr create"`/`git commit -m "..."`(문자열)은 무시. `cd x && gh pr create`(또는 `pushd`)처럼 **cwd를 바꾸는 compound는 skip+advisory** — 훅은 항상 자기 root에서 diff하므로 실제 명령이 도는 디렉터리와 스코프가 어긋날 수 있음(조용히 다른 diff를 리뷰하지 않음). `gh pr edit`은 **게이트 안 함**. **한계(보안 경계 아님 — fail-open, 데이터 경계는 secret-scan+read-only peer)**: heredoc·`$(gh pr create)`·서브셸은 매칭 안 함(skip); 따옴표 내부 `; gh pr create`는 over-match 가능(무해 — 본인 브랜치 diff 리뷰); `git commit && gh pr create`는 PreToolUse가 명령 전 실행돼 **pre-commit HEAD**를 리뷰(advisory 경고 출력).
