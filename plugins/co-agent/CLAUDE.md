...
| `harness-analyst` | hill-climbing 분석가(advisory-only) — `.claude/co-agent-consensus/` 누적 기록(`stage_wall.tsv`·`tasks/*/result.json`·게이트 result)을 읽어 `/co-agent:configure set` 제안 생성. **설정을 직접 쓰지 않음**; 기록 <3회면 제안 없이 관찰만 |
