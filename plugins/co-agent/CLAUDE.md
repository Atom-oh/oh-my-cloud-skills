...
| `pr-autofix` | "pr autofix", "PR 자동 수정", "리뷰 피드백 수정" | PR 생성 후 AI/사람 리뷰를 폴링 → 플랜(Fable/Opus) → 격리 worktree 구현 → 승인 델타만 랜딩 → 커밋·푸시를 루프 (`/co-agent:pr-autofix`; 루프 상한은 `set pr_autofix max_iterations`, 기본 5). 이터레이션 >3은 push 전 co-agent pre-push lens 게이트를 통과해야 하며 미통과 시 재계획(1회), >5는 모델 에스컬레이션(일회성, config 미기록) |
