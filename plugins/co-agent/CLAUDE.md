...
- **데이터 경계**: 팬아웃 전 **full diff**의 추가(`+`)·삭제(`-`)·context 모든 전송 라인 secret-scan(cap 너머/주변 크리덴셜도 포착, 패턴은 AWS AKIA/ASIA·GH·Slack·OpenAI sk-proj-·Anthropic·Google + quoted/unquoted env) — 보이면 3rd-party로 **전송하지 않음**. (삭제-only=시크릿 제거 cleanup PR은 차단 대신 advisory.) 전송 payload만 30KB·라인 경계 절단. **diff는 argv에 안 넣음**(`ps` 노출 없음): stdin 채널(codex/agy/gemini) 파이프, **kiro는 stdin 무시**라 temp 파일+`--trust-tools=fs_read`. peer는 격리 temp 디렉터리를 cwd로 실행.
- **신뢰 한계(read-exfil)**: reviewer는 read-only/sandbox/비-acting(codex `-s read-only`·agy `--sandbox`·gemini `-p`·kiro `--no-interactive`+fs_read만 승인)이라 **쓰기/변경은 막지만 읽기는 가능**. 오염된 diff가 절대경로(`~/.aws/credentials` 등) 읽기를 유도하면 그 내용이 외부 AI로 흘러갈 잔존 위험이 있음(상대경로는 cwd 격리로 차단). 그래서 **opt-in·기본 off**이고 활성화가 곧 동의 — diff 외 민감정보가 있을 수 있는 환경에선 켜지 말 것. (`gemini -p`는 sandbox 플래그가 없는 print-mode라 가장 약한 링크 — agy가 있으면 패널이 agy를 택하고 gemini는 fallback일 때만 쓰임.)
- **명령 매칭**: `gh pr create`가 명령 경계(줄 시작 또는 `;`/`&`/`|`/`&&` 뒤)에 올 때만 — `env `·`VAR=val` prefix와 `gh -R o/r pr create`처럼 gh~pr 사이 플래그도 허용. `cd x && gh pr create`(compound)는 잡고 `echo "gh pr create"`/`git commit -m "..."`(문자열)은 무시. `gh pr edit`은 **게이트 안 함**. **한계(보안 경계 아님 — fail-open, 데이터 경계는 secret-scan+read-only peer)**: heredoc·`$(gh pr create)`·서브셸은 매칭 안 함(skip); 따옴표 내부 `; gh pr create`는 over-match 가능(무해 — 본인 브랜치 diff 리뷰); `git commit && gh pr create`는 PreToolUse가 명령 전 실행돼 **pre-commit HEAD**를 리뷰(advisory 경고 출력).
