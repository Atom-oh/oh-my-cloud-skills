...
After the findings list, emit the constraints as a TOP-LEVEL block (not a list item — the
host's approval/disposition filter iterates findings, and constraints must never be
filtered):

```
constraints: |
  <the constraint block handed to you, verbatim>
```
