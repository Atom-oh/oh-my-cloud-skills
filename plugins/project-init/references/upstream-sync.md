...
- `agents/doc-sync-checker.md` — 모델 티어 로컬 `sonnet`+`effort: low`(upstream `opus`). 기계적 doc 비교/채점이라 판단 깊이를 늘릴 여지가 없고 `/sync-docs`마다 호출됨 — 루트 `CLAUDE.md` 티어 표의 "pure dispatch/scan은 sonnet 유지" 예외에 해당. upstream 반영 권장. **tools**: bare `Bash`
