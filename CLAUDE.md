...
**The upstream-owned files remain a byte-identical mirror of `whchoi98/project-init`**
except `version` in `.claude-plugin/plugin.json` (marketplace-uniform). Do not edit those
sources in place: the next sync wipes the change. A repository-owned `.codex-plugin/`
adapter is generated by `scripts/sync-codex-plugins.py` after sync. Local features that used to live here moved to co-agent
(`pr-autofix`, `decision-reconcile`) or to the root routing table (superpowers hints). It
is available in the Codex marketplace through that adapter. Its Claude manifest
