...
${speakerHtml}`;
    ...
    _renderCompareContent(content) {
        ...
        const toggleBar = `<div class="compare-toggle">${cols.map((c, i) => `<button class="compare-btn${i === 0 ? ' active' : ''}" data-compare="${i}">${this._escapeHtml(c.heading)}</button>`).join('')}</div>`;
        const cards = `<div class="col-2">${cols.map((c, i) => `<div class="card compare-content${i === 0 ? ' compare-highlight' : ''}" data-compare="${i}">${c.body}</div>`).join('')}</div>`;
...
RemarpPreviewPanel._editMode = false;
