...
class RemarpPreviewPanel {
    ...
    static createOrShow(extensionUri, document) {
        ...
        const roots = [extensionUri, docDir];
    ...
    static syncCursor(editor) {
        if (RemarpPreviewPanel.currentPanel && RemarpPreviewPanel.currentPanel._suppressSync) {
            return;
        }
    ...
    async _addIssueToSlide(text, slideIndex) {
        const slides = this._parseSlides();
    ...
    async _removeIssueFromSlide(issueText, slideIndex) {
        const slides = this._parseSlides();
        const slide = slides[slideIndex];
        if (!slide || !issueText) {
            return;
...
<body>
    ...
    _getHtmlForSlide(slide, totalSlides) {
        ...
        const rawRenderedContent = rawRenderedHtml.replace(/(<img[^>]+src=["'])([^"']+)(["'])/g, (match, pre, src, post) => {
...
<body data-slide-index="${slide.index}">
    ...
    _transformByType(slideType, content, slide, fm) {
        switch (slideType) {
            case 'cover':
    ...
    _renderCoverContent(content, fm) {
        ...
        if (h2Match)
            remaining = remaining.replace(h2Match[0], '');
...
${speakerHtml}`;
    ...
    _renderTabsContent(content) {
        ...
        // Build tab bar
        const tabBar = `<div class="tab-bar">${tabs.map((t, i) => `<div class="tab-btn${i === 0 ? ' active' : ''}" data-tab="${t.id}">${this._escapeHtml(t.title)}</div>`).join('')}</div>`;
        // Build tab panels
        const tabPanels = tabs.map((t, i) => `<div class="tab-content${i === 0 ? ' active' : ''}" data-tab="${t.id}">${t.content}</div>`).join('');
    ...
    _renderCodeContent(content) {
        const highlighted = content.replace(/(<pre><code[^>]*(?:class="language-(\w+)")?[^>]*>)([\s\S]*?)(<\/code><\/pre>)/g, (_match, _pre, lang, code, _post) => {
            let hl = code;
            // Keywords
            hl = hl.replace(/\b(import|from|export|default|const|let|var|function|class|return|if|else|for|while|try|catch|async|await|new|this|def|self|print|lambda|with|as|raise|yield)\b/g, '<span class="keyword">$1</span>');
            // Strings
            hl = hl.replace(/(&quot;[^&]*?&quot;|&#39;[^&]*?&#39;|"[^"]*?"|'[^']*?')/g, '<span class="string">$1</span>');
            // Comments
            hl = hl.replace(/(\/\/.*$|#.*$)/gm, '<span class="comment">$1</span>');
            // Numbers
    ...
    _renderCompareContent(content) {
        ...
        const toggleBar = `<div class="compare-toggle">${cols.map((c, i) => `<button class="compare-btn${i === 0 ? ' active' : ''}" data-compare="${i}">${c.heading}</button>`).join('')}</div>`;
        if (isSideBySide) {
            const cards = `<div class="col-2">${cols.map((c, i) => `<div class="card compare-content active${i === 0 ? ' compare-highlight' : ''}" data-compare="${i}">${c.body}</div>`).join('')}</div>`;
...
<div class="slide-body" data-compare-mode="side-by-side">${toggleBar}${cards}</div>`;
            ...
            const cards = cols.map((c, i) => `<div class="compare-content${i === 0 ? ' active' : ''}" data-compare="${i}"><h3 style="color:var(--text-accent);margin-bottom:.5rem">${c.heading}</h3>${c.body}</div>`).join('');
            return `${slideHeading ? `<div class="slide-header">${slideHeading}</div>` : ''}
<div class="slide-body">${toggleBar}${cards}</div>`;
        }
    }
    _renderStepsContent(content, slide) {
        // Extract heading
        let slideHeading = '';
...
<div class="slide-body"><div class="timeline">${timelineHtml}</div></div>`;
    ...
    _renderChecklistContent(content) {
        // Extract heading
        let slideHeading = '';
        let bodyContent = content;
        const hMatch = content.match(/(<h[12][^>]*>[\s\S]*?<\/h[12]>)/);
        ...
        bodyContent = bodyContent.replace(/<ul>([\s\S]*?)<\/ul>/g, (_match, items) => {
            const converted = items.replace(/<li>([\s\S]*?)<\/li>/g, (_m, text) => {
...
<div class="slide-body">${bodyContent}</div>`;
    ...
    _renderQuizContent(content) {
        ...
        bodyContent = bodyContent.replace(/<div class="quiz-option( correct)?"[^>]*>([\s\S]*?)<\/div>/g, (_match, correct, text) => {
...
<div class="slide-body">
    ...
    _renderCardsContent(content, slide) {
        // Extract heading
        const hMatch = content.match(/(<h[12][^>]*>[\s\S]*?<\/h[12]>)/);
        const slideHeading = hMatch ? hMatch[1] : '';
        let bodyContent = hMatch ? content.replace(hMatch[0], '') : content;
...
<div class="slide-body">${stepsHtml}</div>`;
    ...
    _parseBlocks(content, slideIndex = 0) {
            ...
            if (blockMatch) {
                ...
                switch (blockType) {
                    ...
                    case 'right':
                        ...
                        break;
                    case 'col': {
                        const cols = [{ html: this._renderInlineMarkdown(inner, slideIndex), id: `${sid}-col-${colCounter++}` }];
                        while (i < lines.length) {
                            let peek = i;
                    ...
                    case 'cell': {
                        const cells = [{ html: this._renderInlineMarkdown(inner, slideIndex), id: `${sid}-cell-${cellCounter++}` }];
                        while (i < lines.length) {
                            let peek = i;
                            while (peek < lines.length && lines[peek].trim() === '') {
                                peek++;
    ...
    _renderInlineMarkdown(content, slideIndex = 0) {
        let html = content;
        const sid = `s${slideIndex}`;
        let quizCounter = 0;
        let liCounter = 0;
    ...
    _renderMarkdown(content, slideIndex = 0) {
