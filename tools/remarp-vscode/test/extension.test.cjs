...
test('compiled preview permits assets in the containing trusted workspace', async () => {
    const folder = path.join(root, 'deck'); fs.mkdirSync(folder);
    const doc = document(path.join(folder, 'source.remarp.md'), '# Source', true);
    active(doc);
    fs.mkdirSync(path.join(folder, 'slides'));
    fs.writeFileSync(path.join(folder, 'slides/default.html'), '<html><head></head><body>Deck</body></html>');
    vscode.workspace.getWorkspaceFolder = () => ({ uri: uri(root) });
    await commands.get('remarp.previewCompiled')();
    assert.deepEqual(errors, []);
    assert.ok(panels.at(-1).options.localResourceRoots.some(value => value.fsPath === root));
});
