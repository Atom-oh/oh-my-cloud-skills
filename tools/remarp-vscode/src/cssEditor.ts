...
export interface CssOverride {
    target: string;  // e.g. "header", "left", "card:1", "canvas:lambda"
...
export class RemarpCssEditor {
    ...
    parseCssBlock(slideContent: string, slideIndex: number): ParsedCssBlock | null {
        ...
        let startLine = -1;
        let endLine = -1;
        const overrides: CssOverride[] = [];

        let currentTarget: string | null = null;
        let currentProperties: Map<string, string> = new Map();
    ...
    async applyChange(
        ...
        const slideContent = document.getText(new vscode.Range(
            new vscode.Position(slide.startLine, 0),
            new vscode.Position(slide.endLine + 1, 0)
        ));
     ...
     */
    findOrCreateCssBlock(document: vscode.TextDocument, slideIndex: number): { exists: boolean; line: number } {
        const slides = this._parseSlides(document);
        if (slideIndex < 0 || slideIndex >= slides.length) {
