...
| `co-agent` | 멀티-AI 협업 (Kiro/Codex/Antigravity — `agy`) — 리뷰, 의사결정 보조, ADR 협업, `sync-context`; Claude가 의장. 명령: `/co-agent:configure`, `/co-agent:sync-context`, `/co-agent:consensus` |
...
│       └── agentcore-create/
│
├── co-agent/                       # 멀티-AI 협업 (1 에이전트, 1 스킬, 5 명령)
│   ├── .claude-plugin/plugin.json
│   ├── CLAUDE.md
│   ├── agents/
│   │   └── co-agent.md
