...
grep -qF '`PANEL-QUALITY: <cell>=<unsupported>/<total>`' "$WORK/synth-prompt.txt" 2>/dev/null \
  ...
  || fail "synthesize (k) literal PANEL-QUALITY line format survives in synth-prompt.txt" \
       "not found in $WORK/synth-prompt.txt"
# The multi-line backtick pair (`### 🧠 MEMORY / CANDIDATES`) is the one that
# actually triggered the original failure — a backtick that opens on one line and
# closes on the next confuses command substitution differently than a same-line
# pair, so it needs its own check rather than relying on the two single-line
# grep -qF assertions above to stand in for it.
grep -q '`### 🧠 MEMORY$' "$WORK/synth-prompt.txt" 2>/dev/null \
  && pass "synthesize (k) multi-line backtick pair's opening line survives in synth-prompt.txt" \
  || fail "synthesize (k) multi-line backtick pair's opening line survives in synth-prompt.txt" \
       "not found in $WORK/synth-prompt.txt"
grep -q '^   CANDIDATES`' "$WORK/synth-prompt.txt" 2>/dev/null \
  && pass "synthesize (k) multi-line backtick pair's closing line survives in synth-prompt.txt" \
  || fail "synthesize (k) multi-line backtick pair's closing line survives in synth-prompt.txt" \
       "not found in $WORK/synth-prompt.txt"
rm -rf "$WORK" "$BIN" "$ERR"
