...
# Trusted L1 validates structure/inventory without executing the PR generator.
# Generated-byte consistency belongs to the separate unprivileged PR-head job.
ORIGIN=$(mktemp -d); BASE=$(mktemp -d); WORK=$(mktemp -d); LOG=$(mktemp)
...
git -C "$REPO_ROOT" show HEAD:scripts/sync-codex-plugins.py > "$BASE/scripts/sync-codex-plugins.py"
if bash "$SCRIPT" "$BASE" 998 "$WORK" >"$LOG" 2>&1; then
  pass "trusted L1 leaves generated-byte consistency to the PR-head job"
else
  fail "trusted L1 leaves generated-byte consistency to the PR-head job" "$(tail -8 "$LOG")"
fi
if [ ! -e "$MARKER" ] && ! grep -q "stale Codex artifact:" "$LOG"; then
  pass "trusted L1 never executes the PR generator or compares against base-generated bytes"
else
  fail "trusted L1 never executes the PR generator or compares against base-generated bytes" "$(tail -8 "$LOG")"
fi
rm -rf "$ORIGIN" "$BASE" "$WORK" "$LOG"
