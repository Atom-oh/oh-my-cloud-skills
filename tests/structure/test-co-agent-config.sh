...
SHOW=$(CO_AGENT_HOST=claude python3 "$CFG" show --root "$R" 2>&1)
...
CODEX_HOST_SHOW=$(CO_AGENT_HOST=claude python3 "$CFG" show --host codex --root "$R" 2>&1)
...
CODEX_HOST_PANEL=$(CO_AGENT_HOST=claude python3 "$CFG" panel --host codex --root "$R" 2>&1)
assert_eq "kiro-cli claude agy" "$CODEX_HOST_PANEL" "codex-host panel swaps codex for claude and prefers agy"

CO_AGENT_HOST=claude python3 "$CFG" set claude model sonnet --host codex --root "$R" >/dev/null 2>&1
CO_AGENT_HOST=claude python3 "$CFG" set claude effort max --host codex --root "$R" >/dev/null 2>&1
CLAUDE_FLAGS=$(CO_AGENT_HOST=claude python3 "$CFG" flags claude --host codex --root "$R" 2>&1 | tr '\n' ' ')   # flags are newline-delimited
...
NO_GEMINI_FALLBACK=$(CO_AGENT_THIRD_AI=gemini CO_AGENT_HOST=claude python3 "$CFG" panel --root "$R" 2>&1)
assert_eq "kiro-cli codex agy" "$NO_GEMINI_FALLBACK" "CO_AGENT_THIRD_AI is not read — panel is always agy"
CO_AGENT_HOST=claude python3 "$CFG" set gemini enabled false --root "$R" >/dev/null 2>&1 && GS=0 || GS=$?
...
CO_AGENT_HOST=claude python3 "$CFG" set codex model gpt-5-codex --root "$R" >/dev/null 2>&1
CO_AGENT_HOST=claude python3 "$CFG" set codex effort high --root "$R" >/dev/null 2>&1
CODEX_FLAGS=$(CO_AGENT_HOST=claude python3 "$CFG" flags codex --root "$R" 2>&1 | tr '\n' ' ')   # flags are newline-delimited
...
CO_AGENT_HOST=claude python3 "$CFG" set agy model "Gemini 3.1 Pro (High)" --root "$R" >/dev/null 2>&1 && AM=0 || AM=$?
assert_eq "0" "$AM" "agy spaced model accepted (exit 0)"
AGY_FLAGS=$(CO_AGENT_HOST=claude python3 "$CFG" flags agy --host claude --root "$R" 2>&1 | tr '\n' ' ')
...
CO_AGENT_HOST=claude python3 "$CFG" set agy model "Gemini; rm -rf /" --root "$R" >/dev/null 2>&1 && MM=0 || MM=$?
...
CO_AGENT_HOST=claude python3 "$CFG" set agy effort high --root "$R" >/dev/null 2>&1 && GE_RC=0 || GE_RC=$?
assert_eq "2" "$GE_RC" "set agy effort → rejected (exit 2)"

# invalid effort value rejected
CO_AGENT_HOST=claude python3 "$CFG" set codex effort turbo --root "$R" >/dev/null 2>&1 && IE_RC=0 || IE_RC=$?
assert_eq "2" "$IE_RC" "invalid effort value → exit 2"

# disable kiro-cli → dropped from panel, enabled check fails
CO_AGENT_HOST=claude python3 "$CFG" set kiro-cli enabled false --root "$R" >/dev/null 2>&1
PANEL=$(CO_AGENT_HOST=claude python3 "$CFG" panel --root "$R" 2>&1)
assert_eq "codex agy" "$PANEL" "disabled kiro-cli removed from panel"
CO_AGENT_HOST=claude python3 "$CFG" enabled kiro-cli --root "$R" >/dev/null 2>&1 && KI_RC=0 || KI_RC=$?
assert_eq "1" "$KI_RC" "enabled kiro-cli → exit 1 when disabled"
CO_AGENT_HOST=claude python3 "$CFG" enabled codex --root "$R" >/dev/null 2>&1 && CO_RC=0 || CO_RC=$?
assert_eq "0" "$CO_RC" "enabled codex → exit 0 when enabled"

# timeout round-trip
CO_AGENT_HOST=claude python3 "$CFG" set timeout 300 --root "$R" >/dev/null 2>&1
assert_eq "300" "$(CO_AGENT_HOST=claude python3 "$CFG" timeout --root "$R" 2>&1)" "set/get timeout round-trips"

# autosync (sync-on-change) toggle: default off, opt-in
R2=$(mktemp -d "${TMPDIR:-/tmp}/coagentcfg2.XXXXXX")
CO_AGENT_HOST=claude python3 "$CFG" autosync --root "$R2" >/dev/null 2>&1 && AS0=0 || AS0=$?
assert_eq "1" "$AS0" "autosync default off → exit 1"
CO_AGENT_HOST=claude python3 "$CFG" set autosync on --root "$R2" >/dev/null 2>&1
CO_AGENT_HOST=claude python3 "$CFG" autosync --root "$R2" >/dev/null 2>&1 && AS1=0 || AS1=$?
assert_eq "0" "$AS1" "set autosync on → exit 0"
assert_contains "$(CO_AGENT_HOST=claude python3 "$CFG" show --root "$R2" 2>&1)" "autosync on" "show reports autosync on"
CO_AGENT_HOST=claude python3 "$CFG" set autosync bogus --root "$R2" >/dev/null 2>&1 && ASB=0 || ASB=$?
...
CO_AGENT_HOST=claude python3 "$CFG" set codex model 'x; rm -rf ~' --root "$R" >/dev/null 2>&1 && MJ=0 || MJ=$?
assert_eq "2" "$MJ" "model with shell metachars rejected (exit 2)"
CO_AGENT_HOST=claude python3 "$CFG" set codex model '*' --root "$R" >/dev/null 2>&1 && MG=0 || MG=$?
assert_eq "2" "$MG" "model with glob char rejected (exit 2)"
CO_AGENT_HOST=claude python3 "$CFG" set codex model gpt-4.1 --root "$R" >/dev/null 2>&1 && MV=0 || MV=$?
...
assert_eq "272000" "$(CO_AGENT_HOST=claude python3 "$CFG" context-limit codex --root "$R3" 2>&1)" "codex default context-limit 272000"
assert_eq "1000000" "$(CO_AGENT_HOST=claude python3 "$CFG" context-limit kiro-cli --root "$R3" 2>&1)" "kiro-cli default context-limit 1000000"
assert_eq "1000000" "$(CO_AGENT_HOST=claude python3 "$CFG" context-limit agy --root "$R3" 2>&1)" "agy default context-limit 1000000"
CO_AGENT_HOST=claude python3 "$CFG" fits codex 200000 --root "$R3" >/dev/null 2>&1 && F1=0 || F1=$?
assert_eq "0" "$F1" "200K tokens fits codex window (exit 0)"
CO_AGENT_HOST=claude python3 "$CFG" fits codex 812861 --root "$R3" >/dev/null 2>&1 && F2=0 || F2=$?
assert_eq "1" "$F2" "812K tokens exceeds codex window (exit 1)"
CO_AGENT_HOST=claude python3 "$CFG" fits kiro-cli 812861 --root "$R3" >/dev/null 2>&1 && F3=0 || F3=$?
assert_eq "0" "$F3" "812K tokens fits kiro-cli 1M window (exit 0)"
CO_AGENT_HOST=claude python3 "$CFG" set codex context_limit 900000 --root "$R3" >/dev/null 2>&1
CO_AGENT_HOST=claude python3 "$CFG" fits codex 812861 --root "$R3" >/dev/null 2>&1 && F4=0 || F4=$?
assert_eq "0" "$F4" "raised codex context_limit lets 812K fit (exit 0)"
CO_AGENT_HOST=claude python3 "$CFG" set codex context_limit 0 --root "$R3" >/dev/null 2>&1 && CL0=0 || CL0=$?
...
CO_AGENT_HOST=claude python3 "$CFG" set codex model gpt-5.5 --scope user --root "$RR" >/dev/null 2>&1 && US=0 || US=$?
...
assert_contains "$(CO_AGENT_HOST=claude python3 "$CFG" show --root "$RR" 2>&1 | tr '\n' ' ')" "gpt-5.5" "user-scope model appears in effective config"
# repo-local overrides user scope
CO_AGENT_HOST=claude python3 "$CFG" set codex model gpt-4.1 --root "$RR" >/dev/null 2>&1
assert_contains "$(CO_AGENT_HOST=claude python3 "$CFG" show --root "$RR" 2>&1 | tr '\n' ' ')" "gpt-4.1" "repo-local overrides user scope"
# invalid scope rejected
CO_AGENT_HOST=claude python3 "$CFG" set codex model x --scope bogus --root "$RR" >/dev/null 2>&1 && BS=0 || BS=$?
