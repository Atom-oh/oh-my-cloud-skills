...
# A mirrored plugin needs a generated Codex adapter too: its directory existing
# must not be enough to bless a marketplace entry whose manifest went missing.
...
assert_contains "$STALE_OUT" "project-init: no .codex-plugin manifest" "a missing project-init Codex adapter is an ERROR"
