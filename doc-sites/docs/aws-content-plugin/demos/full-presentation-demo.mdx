...
> v1.9.0 토큰 시스템으로 재빌드. 커스텀 캔버스(`:::script`)가 많아 `theme.mode: dark`로 다크 스코프를 사용합니다 — 캔버스는 리터럴 색을 유지(`getComputedStyle`/var 미사용)하고 HTML/CSS는 역할 토큰으로 다크에 적응합니다. 아래는 인터랙티브 Anomaly Detection 밴드 시뮬레이터 슬라이드입니다:

![Anomaly Detection 밴드 시뮬레이터 (v1.9.0 theme-dark)](/img/demos/aiops/aiops-band-dark.png)
