...
Without an explicit order, fragments follow their rendered order. Use `order=N` for multi-column content so the reveal sequence follows the intended reading order. Elements sharing an index can appear together.
