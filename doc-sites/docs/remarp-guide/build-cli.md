...
The parser does not expose `--watch` or `--format`. Use `--help` on the installed script to check its exact interface. The `--lang` flag selects language only in the single-file branch; directory builds ignore it, including `--block` builds. For projects and `sync`, set `lang: en` in `_presentation.md` and keep block-level overrides consistent. Without language metadata, project output defaults to Korean. Language metadata selects output language conventions; it does not translate the source prose.
