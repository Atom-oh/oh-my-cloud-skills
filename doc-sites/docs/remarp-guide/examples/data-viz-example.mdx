...
The Chart.js sections below require an explicitly supplied library; it is not bundled by the presentation framework. Follow [Setup](#setup) before using those snippets.
...
## Complete Example: Metrics Dashboard {#complete-example-metrics-dashboard}
...
## KPI Card Patterns {#kpi-card-patterns}

### Basic KPI Card {#basic-kpi-card}
...
### KPI Grid Layout {#kpi-grid-layout}
...
Chart.js is an external prerequisite. Supply a pinned, reviewed UMD build and reference its actual URL in the authored Remarp source before chart initialization. The path below is a placeholder for an asset you provide; the generator does not copy or load Chart.js automatically. See the [official integration guide](https://github.com/chartjs/Chart.js/blob/master/docs/getting-started/integration.md#script-tag).
...
## CSS-Only Charts {#css-only-charts}
...
### Horizontal Bar Chart (CSS Grid) {#horizontal-bar-chart-css-grid}
...
5. **Progressive reveal** - Use `class="fragment"` with `data-fragment-index` to build up dashboards
