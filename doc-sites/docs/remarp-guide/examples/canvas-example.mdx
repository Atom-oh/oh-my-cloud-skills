...
These examples illustrate syntax and rendering patterns. Use the current [Canvas policy](/docs/remarp-guide/syntax/canvas-dsl) and [theme tokens](/docs/remarp-guide/themes/css-variables) for new decks; large or grouped architectures use HTML/CSS. Sample metrics are illustrative.

This example demonstrates Canvas DSL for creating animated architecture diagrams with step-by-step transitions.

## When to Use Canvas DSL {#when-to-use-canvas-dsl}
...
## Complete Example: Serverless API Pattern {#complete-example-serverless-api-pattern}
...
|-----------|-------------|
| `id` | Identifier for arrow connections; use letters, digits and underscores |
...
Circular nodes use the legacy static syntax. The parser records the label, but the
current Canvas compiler does not draw it. Circles accept neither an ID nor a `step`
attribute; use boxes for visible labels, arrow connections, or progressive reveal.
...
The parser accepts this legacy grouping syntax, but the current Canvas compiler
does not draw its boundary. Use the HTML/CSS alternative below for visible groups.
...
## Step Animation Sequence {#step-animation-sequence}
...
## HTML Alternative for Complex Diagrams {#html-alternative-for-complex-diagrams}
...
- `class="fragment"` with `data-fragment-index="N"`: Sequential reveal (like `step N`)
