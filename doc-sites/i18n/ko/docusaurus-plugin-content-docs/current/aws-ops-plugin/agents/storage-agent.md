...
EBS/EFS, CSI 드라이버, 영구 볼륨과 클레임, StorageClasses, 액세스 모드, 토폴로지, 연결 실패, 처리량, 수명 주기를 다룹니다.

## 진단 방법 {#diagnostic-approach}

Pending PVC는 클래스·프로비저너, 용량, 바인딩 모드, 노드 AZ를 비교합니다. 마운트 오류는 CSI 컨트롤러·노드 로그, IAM, 엔드포인트, 파일 시스템 권한을 점검합니다. 파괴적 변경 전에 스냅샷과 복구에 미치는 영향을 확인합니다.
...
[에이전트 명령어와 의사결정 트리](https://github.com/Atom-oh/oh-my-cloud-skills/blob/main/plugins/aws-ops-plugin/agents/storage-agent.md)
