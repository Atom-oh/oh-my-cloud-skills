...
않아도 PATH에 `claude`가 있어야 합니다. [로컬 점검](commands.md#local-checks)을 참고합니다. 요청 시 수정에는 현재 호스트를 사용할 수 있습니다. 선택적 푸시 시 동기화는 작업 범위가 제한된 Claude CLI 수정 도구로 리비전 기준점을 갱신하고 인덱스를 다시 생성한 뒤 수정한 위키 페이지를 커밋합니다.
