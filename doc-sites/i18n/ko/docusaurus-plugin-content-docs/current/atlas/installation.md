---
sidebar_position: 1
title: "atlas 설치"
---
# atlas 설치

## Claude Code {#claude-code}

```text
/plugin marketplace add Atom-oh/oh-my-cloud-skills
/plugin install atlas@oh-my-cloud-skills
```

저장소 체크아웃에서 로컬로 개발할 때는 다음과 같이 실행합니다:

```bash
claude --plugin-dir ./plugins/atlas
```

## Codex {#codex}

`/plugins`를 사용해 이 저장소의 Codex 마켓플레이스에서 `atlas`를 설치한 뒤 새 스레드를 시작합니다. 패키지는 생성된 `.codex-plugin/skills/` 항목을 불러옵니다. 설치된 스킬 선택기를 사용하거나 원하는 작업을 설명합니다. 이 가이드의 슬래시 명령은 대응하는 Claude 워크플로의 이름입니다.

## 설정 및 검증 {#setup-and-verification}

`/atlas:init`으로 위키 주제를 제안합니다. Claude CLI 없이 변경 불일치를 점검하려면
`atlas_drift.py --json`을 사용합니다. 체크아웃 명령은 [로컬 점검](commands.md#local-checks)를
참고합니다. `atlas_sync.py --dry-run`도 모델 호출과 파일 쓰기를 하지 않지만,
인수 해석 전에 PATH에서 `claude`를 찾을 수 있어야 합니다. 푸시 시 자동 동기화는
기본적으로 꺼져 있으며 별도로 설정한 Claude CLI 수정 도구를 사용합니다.

소스 체크아웃에서는 저장소 루트에서 두 호스트의 패키지를 검증합니다:

```bash
python3 scripts/test-plugins.py
python3 scripts/test-codex-plugins.py
```

예상한 항목이 없으면 플러그인 매니페스트와 생성된 오버레이를 확인합니다. 설치 성공만으로 외부 자격 증명, 다른 AI의 CLI 또는 클라우드 권한이 정상 작동한다고 판단하지 않습니다.

## 제거 {#remove}

Claude Code에서는 `/plugin uninstall atlas@oh-my-cloud-skills`를 사용하고, Codex에서는 `/plugins`에서 해당 항목을 제거합니다.
