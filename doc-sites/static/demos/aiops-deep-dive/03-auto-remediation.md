...
:::notes
{timing: 1min}
마지막 블록입니다. 이번에는 분석 결과를 기반으로 자동 복구하는 방법과, AIOps 성숙도 로드맵을 다룹니다.
수동 복구에서 자동 복구로, 그리고 궁극적으로 자가 치유 시스템까지 — 어떻게 진화할 수 있는지 살펴보겠습니다.
{cue: transition} 먼저 SSM Automation 기반 자동 복구부터 시작합니다.
:::
...
:::notes
{timing: 4min}
자동 복구의 핵심 아키텍처입니다. CloudWatch Alarm → EventBridge → SSM Automation Runbook → 검증 + 알림.
SSM Automation Runbook은 AWS API 호출을 단계별로 실행하는 워크플로우입니다. ECS Runbook 예시를 보시면 — 현재 상태 확인 → 스케일 업 → 안정화 대기 → 메트릭 검증 4단계입니다.
{cue: pause} 자동 복구에서 가장 중요한 건 안전장치입니다. Safety Guard 패턴을 보시면 3가지 체크를 합니다.
첫째, blast radius — 영향 범위가 설정 한도를 넘으면 실행하지 않습니다.
둘째, 동시 실행 제한 — 복구 중에 또 다른 복구가 트리거되면 충돌할 수 있으므로 막습니다.
셋째, 승인 게이트 — 위험도가 높은 작업은 사람 승인을 기다립니다.
그리고 롤백 조건을 사전 정의해서, 복구 후 메트릭이 오히려 나빠지면 자동으로 원복합니다.
{cue: transition} 이런 자동 복구가 프로덕션에서 안전하게 동작하는지 어떻게 검증할까요? Chaos Engineering입니다.
:::
...
:::notes
{timing: 3min}
자동 복구 파이프라인을 프로덕션에 배포하기 전에, Chaos Engineering으로 검증해야 합니다.
AWS FIS — Fault Injection Service는 관리형 카오스 엔지니어링 서비스입니다. 특정 AZ나 리소스를 타겟팅해서 장애를 주입하고, Stop Condition을 설정해서 영향이 커지면 자동으로 중단합니다.
{cue: pause} 검증해야 할 4가지 시나리오를 보시면요.
첫째, Pod 50%를 Kill하고 Auto-scaling이 복구하는지 확인합니다.
둘째, 단일 AZ를 차단해서 Multi-AZ 페일오버가 동작하는지 확인합니다.
셋째, 100ms 네트워크 지연을 주입해서 Circuit Breaker가 정상 동작하는지 확인합니다.
넷째, CPU 스트레스를 주입해서 Anomaly Detection이 감지하고 Auto-scale이 트리거되는지 — 이건 Block 2에서 만든 탐지 파이프라인과 Block 3의 복구 파이프라인을 end-to-end로 검증하는 겁니다.
{cue: question} 카오스 엔지니어링을 프로덕션에서 해야 하나요? 네, 스테이징에서는 프로덕션과 다른 문제를 발견합니다. 단, 반드시 Stop Condition을 설정하세요.
{cue: transition} 자동 복구가 MTTR을 얼마나 줄이는지, 직접 계산해 보겠습니다.
:::
  ...
  <div>
    ...
    <div style="margin-bottom:12px;">
      ...
      <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-top:4px;">
        ...
        <div><div style="font-size:0.7rem; color:var(--text-secondary);">After (AIOps)</div><input type="range" id="detect-after" min="1" max="60" value="3" style="width:100%;"></div>
      </div>
    </div>
    <div style="margin-bottom:12px;">
      <div style="display:flex; justify-content:space-between; font-size:0.85rem;">
      ...
      <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-top:4px;">
        ...
        <div><div style="font-size:0.7rem; color:var(--text-secondary);">After (AIOps)</div><input type="range" id="rca-after" min="1" max="120" value="5" style="width:100%;"></div>
      </div>
    </div>
    <div style="margin-bottom:12px;">
      <div style="display:flex; justify-content:space-between; font-size:0.85rem;">
      ...
      <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-top:4px;">
        ...
        <div><div style="font-size:0.7rem; color:var(--text-secondary);">After (AIOps)</div><input type="range" id="fix-after" min="1" max="120" value="8" style="width:100%;"></div>
      </div>
    </div>
    <div style="margin-bottom:12px;">
      <div style="display:flex; justify-content:space-between; font-size:0.85rem;">
      ...
      <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-top:4px;">
        ...
        <div><div style="font-size:0.7rem; color:var(--text-secondary);">After (AIOps)</div><input type="range" id="verify-after" min="1" max="60" value="5" style="width:100%;"></div>
      </div>
    </div>
  </div>
  <div>
...
:::script
(function() {
  var el = document.querySelector('.slide-deck #mttr-canvas');
  if (!el) return;
  var slide = el.closest('.slide');
  if (!slide) return;
  var ids = ['detect','rca','fix','verify'];
  var colors = ['#FF5C85','#AD5CFF','#FF693C','#FBD332'];
  var colorsAfter = ['#00E500','#41B3FF','#00E500','#41B3FF'];
  var canvas = slide.querySelector('#mttr-canvas');
  var ctx = canvas.getContext('2d');
  function calc() {
    var before = 0, after = 0;
    ids.forEach(function(id, i) {
      var bv = +slide.querySelector('#' + id + '-before').value;
      var av = +slide.querySelector('#' + id + '-after').value;
      before += bv; after += av;
      slide.querySelector('#' + id + '-val').textContent = bv + '분 → ' + av + '분';
    });
    slide.querySelector('#mttr-before').textContent = before + '분';
    slide.querySelector('#mttr-after').textContent = after + '분';
    var bh = Math.floor(before / 60), bm = before % 60;
    var ah = Math.floor(after / 60), am = after % 60;
    slide.querySelector('#mttr-before-hr').textContent = bh > 0 ? bh + '시간 ' + bm + '분' : bm + '분';
    slide.querySelector('#mttr-after-hr').textContent = ah > 0 ? ah + '시간 ' + am + '분' : am + '분';
    var pct = before > 0 ? ((before - after) / before * 100) : 0;
    slide.querySelector('#improvement').textContent = pct.toFixed(0) + '%';
    ctx.clearRect(0, 0, 380, 160);
    var barH = 28, gap = 8, startY = 15;
    var maxVal = before || 1;
    var cumBefore = 0, cumAfter = 0;
    ctx.font = '11px system-ui';
    ctx.fillStyle = 'rgba(255,255,255,0.5)';
    ctx.fillText('Before', 5, startY - 2);
    ctx.fillText('After', 5, startY + barH + gap + barH + gap + 8);
    ids.forEach(function(id, i) {
      var bv = +slide.querySelector('#' + id + '-before').value;
      var av = +slide.querySelector('#' + id + '-after').value;
      var bw = bv / maxVal * 340;
      var aw = av / maxVal * 340;
      ctx.fillStyle = colors[i];
      ctx.fillRect(30 + cumBefore, startY, Math.max(bw, 2), barH);
      ctx.fillStyle = colorsAfter[i];
      ctx.fillRect(30 + cumAfter, startY + barH + gap, Math.max(aw, 2), barH);
      cumBefore += bw; cumAfter += aw;
    });
    ctx.font = '10px system-ui';
    var legendY = startY + barH * 2 + gap * 2 + 20;
    ids.forEach(function(id, i) {
      var labels = ['탐지','분석','복구','검증'];
      var lx = i * 90 + 30;
      ctx.fillStyle = colors[i];
  ...
  ids.forEach(function(id) {
    slide.querySelector('#' + id + '-before').addEventListener('input', calc);
    slide.querySelector('#' + id + '-after').addEventListener('input', calc);
  });
  calc();
})();
:::
...
:::notes
{timing: 4min}
AIOps 도입 전후의 MTTR을 직접 비교해 보겠습니다.
왼쪽에서 각 단계별 시간을 조절할 수 있습니다. Before는 현재 수동 운영, After는 AIOps 도입 후입니다.
{cue: pause} 현재 상태를 보면 — 탐지 15분, 분석 30분, 복구 45분, 검증 15분으로 총 MTTR 105분, 약 1시간 45분입니다.
AIOps 도입 후에는 — Anomaly Detection이 3분 내 탐지, GenAI RCA가 5분 내 분석, SSM Automation이 8분 내 자동 복구, 자동 검증 5분. 총 21분으로 80% 단축입니다.
{cue: question} 현재 팀의 MTTR이 어느 정도인지 아시나요? 슬라이더로 직접 입력해 보시면 AIOps 도입 효과를 바로 계산할 수 있습니다.
오른쪽 차트에서 Before/After 바가 시각적으로 비교됩니다. 각 단계별 색상으로 어디서 가장 큰 개선이 일어나는지 한눈에 볼 수 있습니다.
{cue: transition} 이제 전체적인 AIOps 도입 로드맵을 보겠습니다.
:::
...
:::notes
{timing: 3min}
AIOps 성숙도를 4단계로 정리했습니다.
Level 1 Reactive — 대부분의 조직이 여기 있습니다. 수동 모니터링, 정적 임계값, 수동 대응. MTTR 2-4시간.
Level 2 Proactive — 오늘 Block 1에서 다룬 내용입니다. 통합 관측성 구축, ML 이상 탐지 적용. 복구는 사람 승인이 필요하지만 자동화된 Runbook을 사용합니다. MTTR 30-60분.
{cue: pause} Level 3 Predictive — 오늘 Block 2와 3에서 다룬 내용입니다. GenAI RCA, 예측 기반 선제 조치, Safety Guard가 있는 자동 복구. MTTR 10-20분. AWS가 권장하는 목표 수준입니다.
Level 4 Self-Healing — 미래 비전입니다. 시스템이 스스로 문제를 감지하고, 분석하고, 복구하고, 학습까지 합니다. Chaos Engineering이 자동으로 돌면서 복원력을 지속 검증합니다. MTTR 5분 미만.
{cue: question} 현재 조직이 어느 레벨에 있다고 생각하시나요? 대부분 Level 1-2 사이일 텐데, 오늘 세션 내용을 적용하시면 Level 2-3으로 올라갈 수 있습니다.
{cue: transition} 구체적인 도입 로드맵을 보겠습니다.
:::
...
<div style="display:flex; flex-direction:column; gap:16px; margin-top:16px;">
  <div style="display:grid; grid-template-columns:100px 1fr; gap:16px; align-items:start;">
    ...
    <div style="font-size:0.85rem; line-height:1.8; color:var(--text-secondary);">
      ✅ ADOT DaemonSet 배포 + OTel 자동 계측 적용<br>
      ✅ CloudWatch Logs 비용 최적화 (DEBUG 필터 + 샘플링)<br>
  ...
  <div style="display:grid; grid-template-columns:100px 1fr; gap:16px; align-items:start;">
    ...
    <div style="font-size:0.85rem; line-height:1.8; color:var(--text-secondary);">
      ✅ CloudWatch Anomaly Detection 핵심 메트릭 10개 적용<br>
      ✅ DevOps Guru 활성화 (CloudFormation Stack 단위)<br>
  ...
  <div style="display:grid; grid-template-columns:100px 1fr; gap:16px; align-items:start;">
    ...
    <div style="font-size:0.85rem; line-height:1.8; color:var(--text-secondary);">
      ✅ GenAI RCA 파이프라인 구축 (EventBridge → Lambda → Bedrock)<br>
      ✅ SSM Automation Runbook 작성 (ECS 스케일링, RDS 확장)<br>
  ...
  <div style="display:grid; grid-template-columns:100px 1fr; gap:16px; align-items:start;">
    ...
    <div style="font-size:0.85rem; line-height:1.8; color:var(--text-secondary);">
      ✅ RAG 인시던트 DB 구축 → RCA 정확도 향상<br>
      ✅ 자동 복구 범위 확대 + Safety Guard 고도화<br>
...
{timing: 3min}
6개월 로드맵을 제안드립니다.
Month 1-2는 기반 구축입니다. ADOT 배포하고, 로그 비용 최적화하고, L1 대시보드를 만듭니다. 가장 빠르게 ROI가 나오는 구간입니다 — 비용 절감이 즉시 체감되거든요.
Month 3-4는 AI 탐지 적용입니다. CloudWatch Anomaly Detection을 핵심 메트릭에 적용하고, DevOps Guru를 활성화합니다. 이 시점부터 알림 품질이 크게 올라갑니다.
{cue: pause} Month 5-6이 핵심인데 — GenAI RCA 파이프라인을 구축하고, SSM Automation Runbook을 작성합니다. FIS로 검증까지 마치면 Level 3에 도달합니다.
Month 7 이후는 지속적 고도화입니다. RAG 인시던트 DB를 쌓아서 RCA 정확도를 높이고, 자동 복구 범위를 넓히고, 정기 Chaos Day로 복원력을 검증합니다.
{cue: question} 이 중에서 가장 먼저 시작하고 싶은 단계는 어디인가요?
{cue: transition} 마지막으로 전체 세션을 정리하겠습니다.
:::
...
:::notes
{timing: 2min}
전체 세션을 한 장으로 정리하겠습니다.
Block 1에서 관측성 파이프라인을 구축했습니다 — OTel 기반 데이터 수집, 비용 최적화, 통합 대시보드.
Block 2에서 AI로 분석했습니다 — ML 이상 탐지, DevOps Guru 상관분석, GenAI RCA.
Block 3에서 자동으로 복구했습니다 — SSM Automation, Safety Guard, Chaos 검증.
이 3개를 합치면 AIOps가 완성됩니다. 관측성 → AI 탐지 → 자동 복구.
{cue: pause} 핵심 메시지는 — 한 번에 다 하려고 하지 마시고, 6개월 로드맵처럼 단계적으로 접근하세요. 관측성 기반부터 탄탄히 쌓고, AI 탐지를 붙이고, 마지막에 자동 복구를 올리는 순서입니다.
오늘 세션이 도움이 되셨길 바랍니다. 감사합니다.
:::
