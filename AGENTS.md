<!-- generated-by: co-agent · source: CLAUDE.md · claude-md-sha: 495501e3b3cf · generated-at: 2026-09-11 · DO NOT EDIT — edit CLAUDE.md then run /co-agent sync-context -->
...
## Stack
- **Python 3** (stdlib-first; `defusedxml` for XML), **Bash**, **Node.js** (PptxGenJS deck scripts), **Markdown** (agents/skills/commands), JSON manifests. Docs site = Docusaurus (`doc-sites/`).
...
- `python3 scripts/sync-codex-plugins.py --check` — generated Codex adapters match source metadata.
- `python3 scripts/test-codex-runtime.py` — installs all eight plugins in disposable Codex state and verifies actual discovery (no inference).
...
Codex entry skills, inventories, hook bridges and MCP config are generated under each `.codex-plugin/` from `scripts/codex/` and shared procedures. Do not edit generated copies. All source skills, commands and agents must remain reachable; agent Markdown is a procedure, not a native Codex agent registration. Preserve installed-path resolution and the content-review gate. project-init upstream files stay unchanged except the shared version; regenerate its adapter after upstream sync.
