...
### Changed
- **project-init: pr-autofix model tiering** — fix planning now runs on Fable/Opus (inline when the host already runs a strong tier, otherwise a strong-model subagent) and the implementation is delegated to sonnet subagents that apply the plan verbatim; the host diff-checks the result against the plan before committing
...
### Changed
- **project-init: pr-autofix 모델 티어링** — 수정 계획은 Fable/Opus에서 수립(호스트가 이미 상위 티어면 인라인, 아니면 강한 모델 서브에이전트)하고 구현은 계획을 그대로 적용하는 sonnet 서브에이전트에 위임; 호스트가 커밋 전에 결과를 계획과 diff 대조
