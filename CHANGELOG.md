...
### Added
- **aws-content-plugin: Archify가 다섯 번째 다이어그램 경로로 — Remarp 덱 안의 탐색형/인터랙티브 다이어그램 (ADR-020, [#161](https://github.com/Atom-oh/oh-my-cloud-skills/pull/161))** — [Archify](https://github.com/tt-a1i/archify)(MIT)를 **버전 고정 의존**으로 채택(2.16.0 @ `199360cc`, 포크하지 않음; dirty·핀 불일치 클론은 빌드 실패). 신규 `:::archify` 블록이 Archify architecture JSON 스펙(인라인 또는 경로)을 담으면, 빌드가 핀 고정 클론으로 렌더 → 공식 AWS 아이콘을 렌더 후 주입(`archify_icons.py` — PoC의 로드베어링 계약 2개, 페인트 순서와 `<g transform>` 배치를 코드로 유지; 서비스명 어휘는 `layout_aws.py`의 신규 `ARCH_STEMS` 테이블 한 곳으로, 두 다이어그램 경로가 811-icon 공유 라이브러리 기준으로 동일하게 서비스를 명명) → 포커스 격리 iframe으로 임베드한다 — 화살표 키는 클릭 전까지 덱이 갖고, 서빙된 덱에서는 Esc로 복귀. `export_pptx.py --base-url`은 의도적으로 평면화한다: 슬라이드 이미지는 MAP 상태, 인터랙티브 URL은 스피커 노트에. 가드: `--map`/`icons=` 값은 번들 아이콘 라이브러리 내부로 격리(주입기는 파일 내용을 게시용 아티팩트에 인라인하므로 임의 경로는 유출 벡터), `tests/structure/test-archify-structure-probe.sh`는 Archify 업그레이드가 출력 마크업을 바꾸면 크게 실패한다. PoC 증거+재현: `docs/decisions/poc/adr-020/`; 문서 사이트에는 이 기능으로 그린 파이프라인 데모가 추가되고, 플러그인/루트 라우팅 표와 `docs/reference/review-routing.md`에 등록됨

### Changed
- **탈제약 리라이트가 전 플러그인으로 완료 ([#156](https://github.com/Atom-oh/oh-my-cloud-skills/pull/156) aws-ops, [#160](https://github.com/Atom-oh/oh-my-cloud-skills/pull/160) 나머지 6개)** — aws-content-plugin에서 확립한 R1 생존 테스트를 aws-ops(반대 문제: MUST/NEVER 토큰은 0인데 템플릿 스캐폴딩과 축어적 중복이 과다 — 목표 산문 추가 + 중복 제거로 순 −49줄)와 co-agent/kiro/atlas/agentcore-creator/kiro-power-converter(41개 파일, 순 −373줄; project-init은 upstream 미러라 제외)에 적용. 부정형 지시는 툴/포맷 계약·조용한 실패 함정·보안/PII·스크립트 게이트를 명명할 때만 살아남고, 나머지는 목표 문장이 된다
- **co-agent: pr-autofix 제어 흐름을 명시적 상태 그래프로 ([#158](https://github.com/Atom-oh/oh-my-cloud-skills/pull/158))** — 상태 파일 하나(`.claude/co-agent-consensus/pr-autofix/pr-<N>/state.json`: iteration·max_iter·replanned_this_pass·phase·stop_reason)가 git-grep 파생 카운터와 gate-replanned 센티널을 대체; git log는 진실의 원천에서 Poll 진입 시 교차검증·경고 후 채택하는 복구 신호로 강등되어 rebase·커밋 소실에도 루프가 자가 치유된다

### Fixed
- **atlas: 읽기 측 유출 경로와 `git push --dry-run` 오발동 ([#159](https://github.com/Atom-oh/oh-my-cloud-skills/pull/159))** — 격리된 fixer의 PreToolUse 가드가 Edit뿐 아니라 Read/Grep/Glob도 위키 루트로 제한(covered diff는 이미 stdin으로 도착하므로 기능 손실 없이 read-then-launder 경로 차단); 동기화된 각 문서의 diff에 자체 시크릿 스캔을 스테이징 전에 추가; `--dry-run` 푸시는 더 이상 동기화를 발동하지 않음
- **훅 누수/사문 감사 ([#160](https://github.com/Atom-oh/oh-my-cloud-skills/pull/160))** — 오래전 죽은 `$TOOL_*` env 계약을 읽던 훅들을 stdin-JSON 계약으로 이관; kiro의 고아 `acp-server` 프로세스(ppid=1, 헤드리스 `timeout` kill 잔재)를 SessionStart/Stop 훅이 정리; eval-skills 헤딩 검사가 코드펜스 인지형이 되고 `colors-reference.md`의 raw hex 리터럴 제거(스위트의 유일한 상시 실패였음)
- **co-agent: push 게이트가 긴 camelCase 식별자에 오탐** — unquoted 크리덴셜 분기가 `var token = relationshipTokenGeometry(...)` 같은 평범한 소스 대입에 매치(PR #161의 Archify 뷰어 JS에서 실측); 값에 숫자 또는 `/`,`+` 하나를 요구하도록 수정 — 실제 키 재료에는 사실상 항상 있고 식별자에는 없으며, 모든 true-positive 클래스(AKIA·PAT·base64·quoted 리터럴)는 그대로 커버
- **scripts: sync-plugin-cache.sh가 `set -e`에서 첫 파일에 즉사, 8개 중 3개 플러그인만 커버** — `((COUNT++))`는 첫 증가에서 status 1을 반환; 하드코딩된 플러그인 목록 탓에 나머지 5개(특히 co-agent의 라이브 훅) 캐시 사본이 소스에서 조용히 드리프트했다

## [1.16.0] - 2026-08-18

### Added
- **pre-push 리뷰 게이트(두 플러그인, opt-in·기본 off) + kiro 토글용 SessionStart 라우팅 훅** — `git push`는 내용이 머신을 떠나는 마지막 체크포인트이므로, 두 플러그인 모두 푸시 직전 범위를 리뷰할 수 있게 됐다: co-agent의 `push_gate`는 3개 렌즈(correctness/security/scope)를 패널에 라운드로빈으로 팬아웃해 **BLOCK한 렌즈 수**로 판정(2개 이상 = BLOCKED, 정확히 1개 = CHAIR JUDGMENT REQUIRED — 훅은 Claude를 직접 호출할 수 없으므로 이 stderr가 판정 전달 경로), kiro의 `review.on_push`는 같은 3개 렌즈를 병렬 `kiro-cli` 호출로 돌리며 `push_block` 기본값을 커밋 게이트의 `critical`보다 한 단계 엄격한 `warning`으로 둔다. 둘 다 **기본 off** — 활성화가 곧 외부 백엔드로의 diff 송신 동의 — 이고, `co_agent_config.py`에 kiro의 tracked-file consent 스트리핑을 이식해 repo가 `.claude/co-agent.local.json`에 `pr_gate`/`push_gate`를 켜서 커밋해도 설치한 사용자를 대신 opt-in시킬 수 없다. 두 게이트는 자신이 서술할 수 없는 푸시를 SKIP(fail-open·advisory)한다: 같은 invocation의 선행 `cd`/`pushd`·`git commit`, 다른 repo/워크트리 리다이렉트, ref 삭제, 그리고 `--all`/`--tags`/`--mirror` 또는 명시적 refspec — 이들이 실제로 보내는 내용은 게이트가 diff하는 `@{upstream}...HEAD`(three-dot, merge base 기준)와 무관하다. 둘을 동시에 켜면 매 푸시마다 독립 리뷰 2회가 돈다는 경고가 출력된다. 별도로 `plugins/kiro/hooks/session-routing.sh`는 죽어 있던 토글을 되살린다: 플러그인 자신의 `CLAUDE.md`는 세션 컨텍스트에 실리지 않으므로(프로젝트의 것만 실림) `default_delegate`/`websearch.enabled`는 어떤 소비자 repo에서도 아무 동작을 하지 않았다 — 라우팅 규칙을 SessionStart 훅에서 내보내도록 바꿨고, 그것이 출력이 실제로 컨텍스트에 들어가는 유일한 플러그인측 채널이다
- **kiro: 원래부터 있던 헤드리스 레버 활용 — `--effort`, `--resume-id`, `--require-mcp-startup`, 크레딧 텔레메트리** — 공개 헤드리스 블로그 글은 플래그를 4개만 나열하고, 이 플러그인의 `references/kiro-headless.md`도 그 목록을 전부인 것처럼 반복해 왔다. 2.11.1의 `kiro-cli chat --help`를 실제로 확인하면 `--effort`, `--resume`/`--resume-id`/`--resume-picker`, `--list-sessions --format json`이 모두 존재한다 — 즉 위임 파이프라인이 이들을 안 쓰고 있던 원인은 CLI가 아니라 문서 버그였다. 변경: (1) fix 라운드가 재시작이 아니라 **resume** — 신규 `scripts/kiro_run.py session-id <wt>`가 세션 저장소를 읽고(`cwd`로 그룹화되므로 태스크별 worktree가 자기 세션을 유일하게 식별), 2라운드는 `--resume-id <id>`로 같은 대화를 이어가며 실패한 테스트 출력만 전달, id가 없으면 전체 컨텍스트를 담은 새 호출로 폴백(세션 id 부재가 태스크 실패 사유가 되는 일은 없음); (2) 신규 `delegate.effort`(기본 **`low`** — Claude가 이미 쓴 스펙을 적용하는 작업이므로 `pr-autofix-implementer`와 같은 근거)와 `review.effort`(기본 **`high`** — 차단 판정 자체가 그 호출의 산출물), 둘 다 `/kiro:configure set <delegate|review> effort <low|medium|high|xhigh|max>`이며 `default`는 플래그를 생략, 손으로 잘못 넣은 값은 호출을 깨뜨리는 대신 "플래그 생략"으로 강등; (3) `kiro_run.py credits <log>...`가 파이프라인이 이미 파일로 리다이렉트하는 로그에서 `Credits: <n>` 턴 푸터를 합산(ANSI 제거)해 위임률 리포트에 추가 — best-effort로, 푸터 형식이 바뀌면 추측하지 않고 해당 줄을 생략; (4) delegate 호출에 `--require-mcp-startup`을 추가해 조용히 죽은 MCP 서버를 선행 exit 3으로 드러냄(fix 라운드 대상 실패가 아니라 인프라 실패 → Claude 폴백). `references/kiro-headless.md`의 플래그 절을 실측 기준으로 재작성하고, upstream 요청(kiro#5423, kiro#9066)에 대한 주장도 실제로 없는 것 하나 — turn 단위 이벤트 스트리밍 — 으로 좁혔다

### Changed
- **project-init을 upstream과 byte-identical 미러로 전환; `pr-autofix`·`decision-reconcile`은 co-agent로 이전** — `whchoi98/project-init`에서 12개 파일이 로컬 분기 상태였고, 동기화마다 rsync 제외 목록을 관리해야 했으며 하나만 빠뜨려도 로컬 변경이 소실됐다. 이제 `plugins/project-init/`은 upstream `da91979`(v2.2.0)를 그대로 추적하고, 유일한 로컬 델타는 `.claude-plugin/plugin.json`의 `version`이다. 로컬 전용 스킬 2개는 원래 있어야 할 플러그인으로 옮겼다(둘 다 멀티모델 패널): `pr-autofix`(+ `pr-autofix-planner`/`pr-autofix-implementer` 에이전트, `/co-agent:pr-autofix`)와 `decision-reconcile`. 파생 결과: superpowers 라이프사이클 라우팅 힌트는 루트 `CLAUDE.md` 표에만 존재(플러그인 안에 두면 다음 동기화에서 소실 — `tests/structure/test-superpowers-integration.sh`가 project-init에 힌트가 없음을 검증), 로컬 GitHub-metrics 배지 헬퍼(`fetch_github_metrics.py` + `/generate-readme` Step 2.5)와 project-init의 `.codex-plugin/plugin.json`은 삭제되어 Codex 마켓플레이스에서 제외, `doc-sync-checker`는 upstream의 bare `model: opus`를 유지(미러 파일이므로 `model`+`effort` 규칙의 명시적 예외). `scripts/test-plugins.py`는 매니페스트에 배열이 없으면 디스크에서 에이전트/스킬을 찾아 검증하므로 미러 플러그인도 frontmatter 검증이 건너뛰어지지 않으며, `scripts/test-codex-plugins.py`는 `CLAUDE_ONLY` allowlist에 등재된 플러그인(project-init)만 건너뛰고, 그 외 플러그인의 `.codex-plugin` 매니페스트 부재 — 또는 allowlist 플러그인에 남은 stale Codex 마켓플레이스 엔트리 — 는 여전히 error다. 동기화 절차: `docs/reference/project-init-upstream-sync.md`
- **pr-autofix 반복 상한을 하드코딩 5에서 co-agent 설정으로 전환** — `/co-agent:configure set pr_autofix max_iterations <n>`(기본 5, 양의 정수)이며 스킬이 시작 시 `co_agent_config.py pr-autofix-iterations`로 읽는다. 적정 횟수는 리포의 리뷰 CI가 findings를 얼마나 점진적으로 내놓는지에 달려 있어 스킬 상수가 아니라 리포별 속성이다
...
### Changed
- **project-init: pr-autofix — 반복 5회로 상향(기존 3회), implementer는 opus/medium으로 승격(기존 sonnet/high)** — 리뷰 패널이 지적을 점진적으로 드러내는 루프에서 3회는 너무 빡빡했음(1라운드 피드백을 고치면 그 패널이 아직 안 본 2라운드 지적이 새로 드러나는 경우가 흔함); 상한을 5회로 올림. 별개로 implementer 서브에이전트를 `sonnet`+`high`에서 `opus`+`medium`으로 전환: implementer의 역할은 이미 승인된 계획을 그대로 적용하는 편집 전용 작업이라 reasoning depth(`high`)는 이득이 없지만, opus의 멀티파일 수정 신뢰도가 fix-loop 반복 횟수를 직접 줄여줌 — 계획-승인 수정이 재작업이 필요해지면 서브에이전트 호출 1회가 아니라 전체 poll-fix-push 사이클 하나가 통째로 추가되므로, 여기서는 신뢰도 트레이드오프가 토큰 비용 트레이드오프보다 우선함
- **전체 플러그인: 에이전트별 `effort` frontmatter + DeepSWE 기반 모델 티어링** — 모든 에이전트가 `model`과 함께 reasoning-effort 티어를 선언. DeepSWE v1.1 리더보드 근거(모든 Claude 곡선의 무릎이 HIGH; sonnet은 에이전트 스텝을 2배 이상 소모해 opus HIGH가 점수·비용 모두 sonnet HIGH보다 우위): 판단/종합 게이트 9개는 `opus`+`xhigh`, sonnet에서 승격된 멀티스텝 진단/빌드 워커 11개(eks, network, storage, database, observability, analytics, reactive-presentation, workshop, brochure, architecture/animated-diagram)는 `opus`+`high`, 단일 아티팩트 작성은 `sonnet`+`medium`, 순수 디스패치/스캔은 `sonnet`+`low`; PR #62의 sonnet-worker 규칙을 대체 (루트 `CLAUDE.md` Agent File Format 갱신)
- **project-init: pr-autofix 모델 티어링 + worktree 격리** — 수정 계획은 Fable/Opus에서 수립(호스트가 이미 상위 티어면 인라인, 아니면 강한 모델 서브에이전트)하고 구현은 일회용 git worktree에서 동봉된 편집 전용 `pr-autofix-implementer` 에이전트(tools 강제 — Bash/네트워크 불가)가 수행, 계획은 read-only `pr-autofix-planner` 에이전트; landing 메커니즘 전체를 단계 게이트·단위 테스트된 파이프라인 스크립트(`scripts/land_delta.sh` — 실행 표면 denylist, 클린/봉쇄/등가 게이트, 사용자 편집 보존 rollback)로 추출; 호스트가 worktree diff를 계획과 대조해 승인분만 브랜치에 반영 — 사용자의 미커밋 변경은 implementer의 작업 경로 밖에 있음 (checkout 수준 격리 — 보안 샌드박스는 아님)
...
### Fixed
- **aws-content-plugin: workshop-agent 완료 게이트가 90점 면제 스케일과 모순되던 문제 수정** — Workshop 콘텐츠는 Visual-Testing 면제라 90점 스케일(PASS ≥77)로 판정되는데, 에이전트가 워크플로·Quality Review 섹션에 "PASS (≥85점)"를 하드코딩해 정상 77-84점 PASS를 자기 규칙으로 거부했음; 두 게이트 모두 면제 스케일 참조로 수정 (doc-sites 미러 및 workshop-creator CQP 레퍼런스 동기화)
- **kiro: `/kiro:delegate`가 코드를 아무 곳에도 쓰지 못한 채 조용히 실패하던 문제 수정** — `kiro_setup.py`가 `.kiro/agents/{kiro-implementer,kiro-reviewer}.json`을 Claude Code식 중첩 `preToolUse` hook 구조(`{"matcher","hooks":[{"type","command"}]}`)로 생성했으나 kiro-cli 2.11.1은 평면 구조(`{"matcher","command"}`)를 요구하며 중첩 형태를 `missing field \`command\``로 거부함 — `kiro-cli agent validate`는 이 오류에도 exit 0을 반환해 잘못된 config가 감지되지 않았고, kiro-cli는 조용히 기본 에이전트로 폴백해 헤드리스 모드(기본 에이전트는 자동승인 없음)에서 모든 fs_write/fs_read/execute_bash 호출이 "no user to approve"로 거부됐음. 두 에이전트 템플릿 모두 평면 구조로 수정, `write-agents`는 작성 직후 `kiro-cli agent validate`로 검증(exit code 대신 stderr의 "error" 문자열 확인)해 조용히 안 먹히는 config를 쓰는 대신 명확히 실패하도록 함, `references/spec-format.md`/`SKILL.md`에 태스크 분할 규칙(태스크 1개 = 레이어 1개, 파일 ≤5개, 의존순 정렬)을 명시해 멀티레이어 요청이 `delegate.timeout`에 걸려 죽는 거대 태스크 하나로 넘어가지 않고 오케스트레이터가 먼저 분해하도록 함. 후속: 신설한 검증 로직 자체가 `kiro-cli` 실행 중 timeout/OSError가 나면 fail-open됐고(PATH 부재일 때만 fail-open이어야 함), `--force` 없는 재실행 시 디스크에 이미 있는 파일은 재검증 없이 skip — 사용자가 앞선 ❌을 보고 취할 자연스러운 대응(재실행)이 정확히 이 검증이 막으려던 "성공처럼 보이는 bad config"를 재현했음. 이제 kiro-cli가 PATH에 있으면 둘 다 fail-closed로 전환, skip-exists 경로도 기존 파일을 재검증하도록 수정. 2차 후속: exit-0 성공 판정이 stderr만 검사해 exit-nonzero 분기(양쪽 스트림 검사)와 비대칭이었음 — kiro-cli가 검증 오류를 stdout에 찍으면서 exit 0을 반환하는 빌드가 나오면(이 함수가 원래 우회하려던 "exit code를 못 믿는" 동작 그 자체) 그대로 통과했을 것. 이제 양쪽 스트림을 모두 검사하고, 특정 문자열 매칭 대신 "출력이 하나라도 있으면 실패"(정상 파일은 stdout·stderr 모두 빈 문자열로 확인됨)로 판정해 향후 kiro-cli가 오류 문구를 바꾸거나 지역화해도 검증이 무력화되지 않도록 함. 3차 후속: delegate 파이프라인이 실제로 신뢰하는 `verify_agents()`가 디스크 파일을 generator의 dict 출력과 등가 비교만 했음 — `write_agents` 자신이 이미 INVALID로 판정한 파일(generator 출력과 바이트 단위로 동일하지만 kiro-cli가 거부 — 정확히 이 버그의 형태)도 `verify_agents`의 dict 등가 검사는 통과시켜 그대로 신뢰될 수 있었음; `verify_agents`도 이제 동일한 `kiro-cli agent validate` 검증을 재실행해, 이 전체 수정이 막으려던 silent fallback이 되돌아올 수 있는 마지막 경로를 닫음
