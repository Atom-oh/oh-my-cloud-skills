...
### Changed
- **project-init을 upstream과 byte-identical 미러로 전환; `pr-autofix`·`decision-reconcile`은 co-agent로 이전** — `whchoi98/project-init`에서 12개 파일이 로컬 분기 상태였고, 동기화마다 rsync 제외 목록을 관리해야 했으며 하나만 빠뜨려도 로컬 변경이 소실됐다. 이제 `plugins/project-init/`은 upstream `da91979`(v2.2.0)를 그대로 추적하고, 유일한 로컬 델타는 `.claude-plugin/plugin.json`의 `version`이다. 로컬 전용 스킬 2개는 원래 있어야 할 플러그인으로 옮겼다(둘 다 멀티모델 패널): `pr-autofix`(+ `pr-autofix-planner`/`pr-autofix-implementer` 에이전트, `/co-agent:pr-autofix`)와 `decision-reconcile`. 파생 결과: superpowers 라이프사이클 라우팅 힌트는 루트 `CLAUDE.md` 표에만 존재(플러그인 안에 두면 다음 동기화에서 소실 — `tests/structure/test-superpowers-integration.sh`가 project-init에 힌트가 없음을 검증), 로컬 GitHub-metrics 배지 헬퍼(`fetch_github_metrics.py` + `/generate-readme` Step 2.5)와 project-init의 `.codex-plugin/plugin.json`은 삭제되어 Codex 마켓플레이스에서 제외, `doc-sync-checker`는 upstream의 bare `model: opus`를 유지(미러 파일이므로 `model`+`effort` 규칙의 명시적 예외). `scripts/test-plugins.py`는 매니페스트에 배열이 없으면 디스크에서 에이전트/스킬을 찾아 검증하므로 미러 플러그인도 frontmatter 검증이 건너뛰어지지 않으며, `scripts/test-codex-plugins.py`는 Claude 전용 플러그인을 error가 아니라 warning으로 보고한다. 동기화 절차: `docs/reference/project-init-upstream-sync.md`
