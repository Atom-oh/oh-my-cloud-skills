...
- [x] Assert `ops-troubleshoot/SKILL.md` description names `systematic-debugging`.
- [x] Assert `aws-ops-plugin/CLAUDE.md` has a routing note naming `superpowers:systematic-debugging`
...
- [x] Assert root `CLAUDE.md` has a routing-table row mapping `systematic-debugging` → `ops-troubleshoot`.
- [x] Run `bash tests/run-all.sh` — the new assertions FAIL (red). Confirm failure is the new ones.
...
- [x] Description: append "— the AWS/EKS domain arm of `superpowers:systematic-debugging`".
- [x] Add a short body section: when invoked from systematic-debugging, we supply domain
...
- [x] Re-run test — Task 1's first assertion passes.
...
- [x] Add "superpowers handoff" note: inside `superpowers:systematic-debugging`, when the failing
...
- [x] Bilingual (KO/EN). Re-run test — the CLAUDE.md assertion passes.
...
- [x] Add "## superpowers Integration Routing" with a table; first row:
...
- [x] Re-run test — root CLAUDE.md assertion passes (all green).

### Task 5: Verify + commit

- [x] `bash tests/run-all.sh` — all green (new + existing).
- [x] Commit (explicit paths): the test + 3 edited files, one task-scoped message.
