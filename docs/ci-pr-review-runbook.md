...
2. **L1 통과** 시: PR 코멘트의 `_Cells (model/lens):_` 줄에 `codex/L2`, `kiro-opus/L3`,
   `kiro-minimax/L4`, `kiro-glm/L5` 등 최대 16개 `<모델>/<lens>` 태그가 보이면 정상(일부 셀은
...
- kiro-cli: `claude-opus-4.8`/`minimax-m2.5`/`glm-5` 각각 L2~L5 당 1회, 총 12콜. (`kimi-k2.5`는
  프로덕션에서 커버리지 저하 2/2회 + 근거 없는 지적 7건으로 교체됨. **주의**: `gpt-5.5`는
  `kiro-cli --list-models`에 나열되지만 실제 호출은 `INVALID_MODEL_ID`로 거부됨(`[Internal]`
  태그 모델은 목록에 있어도 프로비저닝 안 됐을 수 있음 — codex 는 Bedrock 경유라 무관, kiro-cli
  자체 라우팅만 막힘) — 이 kiro-cli 계정에선 쓰지 말 것. 아래 "특정 모델이 계속
  flaky" 절 참조.)
