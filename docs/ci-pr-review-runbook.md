...
- codex: `openai.gpt-5.6-sol` (bedrock-mantle, In-Region us-east-1; 이미지 `~/.codex/config.toml`의 region이 결정) — L2~L5 각 lens 당 1회, 기본 활성 로스터 기준 총 4콜. (`gpt-5.5`→`gpt-5.6-sol` deprecation 교체 — ADR-014.)
