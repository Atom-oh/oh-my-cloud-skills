...
- **Next number**: inspect `rg --files docs/decisions -g 'ADR-*.md' | sort -V` for the highest
  zero-padded `ADR-NNN-*.md` and increment it. ADR-022 is the latest as of
...
[ADR-021](ADR-021-english-docs-current-review-authority.md) records the current English
documentation and review authority. Earlier model rosters, plugin totals, timeout
