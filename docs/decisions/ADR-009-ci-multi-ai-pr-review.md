...
- **Security** — Because `pull_request_target` runs with secrets/write context, the **base (trusted) branch's scripts** are checked out, and the PR's changes are received only as data via `gh pr diff` (never executed). → Script modifications therefore take effect only starting from the PR that merges them.
