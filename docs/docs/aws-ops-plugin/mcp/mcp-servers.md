...
AWS Ops Plugin에서 사용하는 MCP 서버를 설명합니다. 이 플러그인은 2개의 MCP 서버를 번들로 포함하며, 나머지 3개는 `deploy-on-aws` 플러그인에서 제공됩니다.

## 개요

| 서버 | 소스 | 타입 | 용도 | 사용 에이전트 |
|------|------|------|------|---------------|
| `awsdocs` | **this plugin** | stdio/uvx | AWS 공식 문서 검색/읽기 | 모든 에이전트 |
| `awsapi` | **this plugin** | stdio/uvx | AWS API 직접 호출 (describe, list 등) | eks, network, iam, storage, database, observability, analytics |
| `awsknowledge` | deploy-on-aws | HTTP | AWS 아키텍처 지식, 권장사항, 리전 정보 | 모든 에이전트 |
| `awspricing` | deploy-on-aws | stdio/uvx | 비용 분석, 가격 조회 | cost-agent |
| `awsiac` | deploy-on-aws | stdio/uvx | CloudFormation/CDK 검증, 트러블슈팅 | eks-agent, ops-coordinator |
...
### awsknowledge (deploy-on-aws)

AWS 아키텍처 지식과 모범 사례를 제공하는 HTTP 기반 MCP 서버입니다. `deploy-on-aws` 플러그인에서 제공됩니다.
...
### awspricing (deploy-on-aws)

AWS 서비스 가격 정보를 조회하고 비용을 분석하는 stdio 기반 MCP 서버입니다. `deploy-on-aws` 플러그인에서 제공됩니다.
...
### awsiac (deploy-on-aws)

CloudFormation 및 CDK 템플릿을 검증하고 트러블슈팅하는 stdio 기반 MCP 서버입니다. `deploy-on-aws` 플러그인에서 제공됩니다.
