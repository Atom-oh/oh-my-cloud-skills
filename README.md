...
- **Panel of installed CLIs** — fan the same prompt to Kiro/Codex/Antigravity in parallel; Claude chairs and synthesizes consensus vs. dissent (degrades gracefully if none installed)
...
| `co-agent` | Multi-AI collaboration (Kiro/Codex/Antigravity) — review, decision support, ADR co-authoring, and `sync-context`; Claude chairs. Commands: `/co-agent:configure`, `/co-agent:sync-context`, `/co-agent:consensus` |
...
Co-agent collab:     prompt  -->  fan-out to Kiro/Codex/Antigravity  -->  Claude synthesizes  -->  review / decision / ADR / sync-context
