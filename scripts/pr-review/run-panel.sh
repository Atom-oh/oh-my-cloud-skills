...
for lens_file in "${LENS_FILES[@]}"; do
  ...
  # ~/.codex/config.toml 이 결정하며, 그 값이 global.openai.gpt-6-astra; kiro-gpt 셀의
  # gpt-5.6-terra 와는 별개 문자열 — Kiro 의 cross-vendor 라우터 카탈로그와 Codex 자체
  # amazon-bedrock-runtime 카탈로그가 서로 다른 alias 를 매핑하므로, 하나가 다른 하나의
  # 오타/drift가 아니다. ADR-014). --skip-git-repo-check 필수. global.openai.gpt-6-astra 는
  # amazon-bedrock-runtime 을 통한 global 모델이라 이전 gpt-5.6-sol/bedrock-mantle 과 달리
