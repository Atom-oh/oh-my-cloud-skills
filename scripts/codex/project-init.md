...
untrusted hooks ran. For a non-Git project, resolve script paths for that project's
layout before registering hooks. Add external notifications only when the user
...
do not copy the source's hard-coded `/200` denominator (its listed weights sum to
195). Replace Claude-specific checks with the corresponding Codex checks.
